...
if (isset($_SESSION["num_ganador"])) {
	...
	//Inventar un número ganador, y lo guardo en la variable de sesión, para que esté disponible para futuras tiradas, en caso de que el jugador no acierte. Inicializo intentos, máximo y mínimo:
...
if (isset($_POST["inp_num_jugador"])) {
	...
	if ($num_jugador == $num_ganador) {
		...
		$_SESSION["max"]=$num_jugador;
	}else{
		$_SESSION["min"]=$num_jugador;
	}
}
...
?>
	<center>
		...
		<h3>RESULTADO !!!</h3>
		<?php
		echo "<h4>Num jugador: $num_jugador</h4>";//   ---    Num ganador: $num_ganador
		if ($acierto == true) {
